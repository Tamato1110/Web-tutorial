...
function preload() {
  // Step 1.1 code goes here//載入所有的圖片待用
  ...
  this.load.spritesheet('dude',
    ...
    { frameWidth: 32, frameHeight: 48 }//固定人物的尺寸
  )
...
function create() {
  // Step 1.2 code goes here//在遊戲開始的時候創造地圖
  ...
  // Step 4.3 code goes here
    this.physics.add.collider(player, platforms)//讓踏板有碰撞體積
  // Step 5.1 code goes here
  stars = this.physics.add.group({ key: 'star', repeat: 11, setXY: { x: 12, y: 0, stepX: 70 } })//快速生成11顆星星
  stars.children.iterate(star => star.setBounceY(Phaser.Math.FloatBetween(0.2, 0.3)))//iterate注意！,Math注意！
...
function update() {
  ...
  const speed = 160
  if (cursors.left.isDown) {//變數在create裡面宣告，但在update裡面使用
  ...
  if (cursors.up.isDown && player.body.touching.down)//角色可以上跳&&腳下要有平台才能跳
...
function collectStar(player, star) {
  ...
  if (0 == stars.countActive(true)) {
    stars.children.iterate(star => star.enableBody(true, star.x, 0, true, true))//回收星星
